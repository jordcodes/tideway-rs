...
#[cfg(feature = "sessions")]
use tideway::session::{CookieSessionStore, InMemorySessionStore, SessionConfig};
#[cfg(feature = "sessions")]
use tideway::{SessionData, SessionStore};
...
async fn main() -> tideway::Result<()> {
    ...
    let memory_store: Arc<dyn SessionStore> =
    ...
    let cookie_store: Arc<dyn SessionStore> = Arc::new(CookieSessionStore::new(&cookie_config)?);
    ...
    cookie_store
        .save("cookie-session-xyz", cookie_session)
        .await?;
