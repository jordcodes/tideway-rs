...
pub const RESOURCE_API_FLOW: &str = "`tideway resource <name>`";
...
pub const GREENFIELD_PRIMARY_PATH: &str = "For greenfield apps, use the primary path: tideway new <app> -> `tideway dev --fix-env` -> `tideway resource <name>`";
pub const GREENFIELD_NEW_APP_PRESET_API: &str =
    "For greenfield apps, run `tideway new <app> --preset api` first.";
pub const PRIMARY_PATH_REMINDER_CHAIN: &str = "Primary path reminder: tideway new <app> -> `tideway dev --fix-env` -> `tideway resource <name>`";
pub const PRIMARY_PATH: &str =
    "Primary path: tideway new <app> -> `tideway dev --fix-env` -> `tideway resource <name>`";

pub const PRIMARY_PATH_SEQUENCE_PLAIN: &str =
    "tideway new <app> -> tideway dev --fix-env -> tideway resource <name> -> tideway migrate";
pub const CLI_ADVANCED_NOTE: &str =
    "Advanced commands are for existing projects or nonstandard workflows.";
pub const CLI_HELP_TRAILER: &str = "Primary commands: new, dev, resource, doctor, migrate.\nAdvanced commands are for existing projects or nonstandard workflows.\nResource shape flags are for intentional customization.\nPrimary path (recommended): tideway new <app> -> tideway dev --fix-env -> tideway resource <name> -> tideway migrate";
